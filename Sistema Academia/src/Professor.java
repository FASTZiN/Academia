...
public class Professor {
    ...
    void removeAluno(JFormattedTextField matricula) {
      ...
      public void criarNovoExs(JFormattedTextField matricula, JFormattedTextField x_treino,JButton add,JFormattedTextField qtsExs, JFormattedTextField exs, JFormattedTextField serie, JFormattedTextField repeticoes){
    	  for(int i = 0; i < n; i++){
      		  if(this.matriculaExiste(matricula) == true){
      			   //x = alunos[i];
      			alunos[i].criarNovosExs(x_treino, eg, add, qtsExs, exs, serie, repeticoes);
      		  }
      	  }
      }
